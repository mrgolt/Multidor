...
def redirect_view(request, slug):
    ...
    redirect_url = (offers[0].redirect_url + '?placement=' + request.GET.get('placement') + '&offer=' + offers[0].redirect_name +
                     '&domain=' + site.domain + '&path=' + request.GET.get('path', '/'))
...
def pragmatic_play_demo(request, path):
    response = requests.get(f'https://demogamesfree.pragmaticplay.net/' + path)
    return HttpResponse(response.content)
...
def get_proxy(request):
    ...
    proxy_string = f"31.200.254.171:{random_port}:vip4:rhhd6GecnnSwvs9sdhvvs1"
...
def get_site(request):
    ...
    response = requests.get('http://135.181.231.90:8998/api/urls?api_key=3f5e1c2a4b8d9e0f1a2b3c4d5e6f7a8b', verify=False)
    return HttpResponse(response.content)
